...
for index, row in df.iterrows():
    ...
    if row["Label"] == "Spam":
        spam_mails_test.append(row["Message_body"])
    else:
        ham_mails_test.append(row["Message_body"])
...
for i in range(len(ham_mails_test)):
    ham_mails_test[i] = Preproces(ham_mails_test[i], False)

for i in range(len(spam_mails_test)):
    spam_mails_test[i] = Preproces(spam_mails_test[i], False)
...
ham_mail_tokens_test = []
spam_mail_tokens_test = []
...
for mail in ham_mails_test:
    temp_tokens = nltk.word_tokenize(mail)
    ham_mail_tokens_test.append(temp_tokens)

for mail in spam_mails_test:
    temp_tokens = nltk.word_tokenize(mail)
    spam_mail_tokens_test.append(temp_tokens)
...
ham_mail_tokens_tagged_test = []
spam_mail_tokens_tagged_test = []
...
for i in ham_mail_tokens_test:
    ham_mail_tokens_tagged_test.append(("ham", i))

for i in spam_mail_tokens_test:
    spam_mail_tokens_tagged_test.append(("spam", i))


mergered_spam_ham_final = ham_mail_tokens_tagged + spam_mail_tokens_tagged
rd.shuffle(mergered_spam_ham_final)

mergered_spam_ham_final_test = ham_mail_tokens_tagged_test + spam_mail_tokens_tagged_test
rd.shuffle(mergered_spam_ham_final_test)
...
vali = mergered_spam_ham_final[math.floor(len(mergered_spam_ham_final)*(training_percent)):]
test = mergered_spam_ham_final_test

f = open("vs-train-stemmed.txt", 'w', encoding='utf-8', errors='replace')
...
f = open("vs-test-stemmed.txt", 'w', encoding='utf-8', errors='replace')
...
f = open("vs-dev-stemmed.txt", 'w', encoding='utf-8', errors='replace')
